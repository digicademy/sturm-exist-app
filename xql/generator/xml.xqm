                ...
                for $year in xmldb:get-child-collections(concat($sourceDir, $artist, '/'))
                    let $fileItems := 
                        for $file in collection(concat($sourceDir, $artist, '/', $year, '/'))
                            ...
                            let $fileItem := 
                                <item xmlns="http://www.tei-c.org/ns/1.0" source="{data($file//tei:msDesc/tei:idno[@type='uri'])}" xml:id="{$file/tei:TEI/@xml:id}" n="letter">
                                    <ref target="{$itemName}"><time when="{data($file//tei:correspAction[@type='sent']/tei:date/@*)}">{data($file//tei:correspAction[@type='sent']/tei:date/@*)}</time>, <placeName>{data($placeName)}</placeName></ref>,<ref target="{data($file//tei:msDesc/tei:idno[@type='uri'])}">Sturm Archiv I, {data($file//tei:correspDesc/@key)}</ref>
                                </item>
                        return $fileItem
...
declare function sturm_xml:Cmif($appRoot as xs:string, $appName as xs:string, $source as xs:string, $target as xs:string) {
    ...
    let $transformationParameters := 
        <parameters>
            ...
            <param name="title" value="Der STURM. Digitale Quellenedition zur Geschichte der europäischen Avantgarde. Abteilung I, Künstlerbriefe" />
